...
def slave_create(node_name, working_dir, executors, labels):
    j = Jenkins(os.environ['JENKINS_URL'], username=os.environ['JENKINS_USER'], password=os.environ['JENKINS_PASS'])
...
# print("jenkinsapi Version:" + jenkinsapi.__version__)
...
if os.environ['CSRF_PROTECTION_ENABLE'] == 'true':
    print("CSRF Protection Enable")
    # CSRF Protection.
    # For more info visit: https://wiki.jenkins.io/display/JENKINS/Remote+access+API#RemoteaccessAPI-CSRFProtection
    # req = requests.get(os.environ['JENKINS_URL'] + '/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)', auth=(os.environ['JENKINS_USER'], os.environ['JENKINS_PASS']))
    #crumb=CrumbRequester(username=os.environ['JENKINS_USER'], password=os.environ['JENKINS_PASS'], baseurl=os.environ['JENKINS_URL'])
    #print("Crumb obtained: " + crumb)
